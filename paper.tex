...
As shown in Figure~\ref{fig-arg-data-structure}B,
...
tasks. The vitally important and fundamental distinction between
a gARG and the classical eARG perspective is that gARGs do not
\emph{require} us to posit explicit recombination events in order
to describe a dataset, but
instead provide a means of encoding the effects of
multiple events, and to systematically describe the uncertainty around them
(see Section XXX).
...
sequences.
For example, Figure~\ref{fig-arg-in-pedigree}
...
trees~\citep[e.g.][]{minichiello2006mapping,griffiths1996ancestral} [more]
