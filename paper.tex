...
and further discussion on the potential impacts.
...
% TODO rephrase, not great currentyl
We argue that the prevailing definition of an ARG
imposes unneccesary limitations, and that with a broader definition
...
The pedigree relationships between the diploid individuals is
...
adding concrete details to the mechanisms by which
differential inheritance of genetic material are modelled.
...
the corresponding crossover breakpoint, and these breakpoints
are used to reconstruct the local trees.
...
or (e.g.) multiple recombination nodes joined by a zero-length edge.
...
denoting the genomic intervals ancestral to a set of samples
on the edges of an ARG, is a key concept. In
Fig.~\ref{fig-arg-in-pedigree}, for example, we have
four sample genomes, \noderef{a}--\noderef{d}. As we
trace their genetic ancestry into the previous generation
(\noderef{e}--\noderef{h}), we can think of the ancestral
material of the sample propagating through the graph
backwards in time. In the interval $[2, 7)$, there is a
local coalescence where nodes \noderef{a} and \noderef{c}
find common ancestor in \noderef{f}. Thus, in this interval,
...
the information in the input eARG. They are also also analogous to the
