...
from the graph.
The ``grand MRCA'' \noderef{q} is normally omitted from the
...
ARGs given the data and a fixed model (the SMC$^\prime$ \citep{marjoram2006fast},
...
have been ``pushed down'' onto a single more recent node.
...
As for coalescence in these inferred ARGs, the shading (``coalescent span'')
...
We can see that both \tsinfer and (to a lesser extent) \relate have edges
that span multiple tree boundaries, indicating that they certainly
do not infer a series of \emph{unrelated} local trees.
...
the entire sequence. They do not, however, show extensive node sharing in the
oldest time periods, reflecting the fact that they do not encode topology
above the local tree roots (i.e. they do not go back to a
"grand MRCA"). This is also seen in the fact they their ARGs contain multiple roots
(nodes with zero parents, in blue), whereas \kwarg and \argweaver have a
single ARG root.
These two methods contain a level of detail about recombination
