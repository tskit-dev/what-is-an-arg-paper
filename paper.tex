...
to describe can be awkward in practise. For example,
...
but in practise methods such has
...
ARG inference methods [cites] , they are highly questionable in
modern datasets. For example, there are now many
human datasets consisting of hundreds of thousands
of
...
While the coalescent model is suprisingly robust to such
basic violations to the underlying assumptions~\citep{wakeley2012gene},
the combination of a large sample size and long sequence length
can produce pathological behaviour in
simulations~\citep{bhaskar2014distortion,nelson2020accounting}.
