...
We show it can fully capture the richness of modern large-scale datasets
...
ARG-unary nodes occur when an ARG node has one child and one or
more parents.
Nodes with one child and one parent (``pass-through'' nodes)
occur where we wish the record the passage of ancestral material
...
ARG-unary nodes are also occur in inferences
from longitudinal datasets where genetic
...
Locally unary nodes are a common and important feature
...
hermaphrodite eukaryote with partially overlapping generations. Here, events are not represented by a single
