...
process~\citep{griffiths1991two,ethier1990two,griffiths1996ancestral,griffiths1997ancestral}
which modelled the coalescent with recombination \citep{hudson1983properties}.
Subsequently, it has come instead to be thought of as a data
structure~\citep{minichiello2006mapping}, i.e.\ a means of
describing the sequence of correlated local trees that
are the \emph{outcome} of some such ancestral process.
However, the distinction between process and structure is not clear cut
...
However, this broad usage means there is disagreement on the
...
Perhaps most importantly, some definitions of ARGs
(especially those linked to a specific stochastic process) are overly narrow, which
...
the output of all popular simulation and inference methods,
yet simple enough to be encoded via a conventional mathematical graph.
...
We follow the well established principle that it is advantageous
to decouple the presence of mutations from the genealogical structure
itself \citep[contra][chap.~3]{gusfield2014recombinatorics}.
We propose a definition of genetic genealogy purely in terms of specific genomes
...
% FIXME WE're missing the important distinction here that the pedigree
% has one node per ind, whereas a gARG has two nodes.
We can therefore view a gARG for a given set of individuals
as a subset of their pedigree.
An example of this for diploids
(so each individual is represented by two nodes)
is illustrated in Fig.~\ref{fig-arg-in-pedigree}.
...
interval $[0, 2)$ from \textsf{e} and it inherits $[2, 10)$ from \textsf{f};
...
% FIXME - the oriented forest definition is wrong, we need to be more
% concrete about the fact that nodes are integers.
Formally, we define a gARG as a sets of nodes $N$ and edges $E$.
...
at position $x$ as an oriented forest $\pi = ( \pi_1, \hdots, \pi_{|N|} )$,
where the entry $\pi_j$ is the index
...
We use a forest to describe the local ancestry at a given
position so that we allow for cases where not all leaves have
found a most recent common ancestor, resulting in multiple roots.
To construct $\pi$ at position $x$, set
$\pi_i = \emptyset$ for $i \in \{1, \hdots, |N|\}$.
% FIXME - this is wrong
...
In this section we have considered the nodes in a gARG as separate from events
in a generative process. Nevertheless, gARGs are commonly constructed to
include only those genomes associated with specific events. For example,
\cite{mathieson2020ancestry} define an ARG as ``a subset of the pedigree...
...
a recombination or coalescence event.''. In an eARG however, events are even
more explicitly embedded in the data structure.
...
% FIXME this isn't worth getting into
...
% JK: previous heading was jargony - this is clearer
...
``unary'' nodes in the local trees
(where a node has exactly one child over a particular genomic region).
% "unary regions" is jargon - just dropping it in here like this doesn't
% clarify. We can't assume that people are used to switching between
% graphs and trees.
