...
\affil[6]{Department of Genetics, Stanford University School of Medicine, Stanford, CA 94305, USA}
...
Originally, the ARG was defined as a variant 
of the coalescent with recombination stochastic 
process~\citep{griffiths1991two,ethier1990two,griffiths1996ancestral,griffiths1997ancestral},
but has subsequently come to be seen as a data
structure~\citep{minichiello2006mapping}, i.e., a means of 
describing the \emph{outcome} of some such ancestral process
(the distinction, however, is not clear cut and subfields use the term
...
avoid describing the output of the methods as an ARG, the general 
consensus has emerged that they belong to a growing class of 
ARG inference methods~\citep[MORE][]{mathieson2020ancestry},
and we therefore need a more general definition of an ``ARG'' than the classical
Griffiths formulation.

A general term to describe the detailed genetic genealogy of a sample 
that encompasses the output of all these methods is indeed useful, but 
there are substantial problems emerging from the current usage of the term 
ARG (and its implicit concrete definition in the Griffiths formulation).
The problem is much deeper than some pedantic discussion over terminology,
and directly informs what we should be attempting to estimate, how we 
evaluate inference methods, and what the limitations on the precision
and accuracy of our inferences are.
We currently lack a well-defined target for inference, and therefore there is 
disagreement on how these inference should be evaluated, and 
what the criteria for success should be.
We also lack a shared format for interchanging ARG data between
programs, with the assorted negative consequences which
geneticists are only too familiar with~\citep{excoffier2006computer}.
Finally, and perhaps most importantly, and overly narrow definition of ARGs
leads to potentially missed opportunities in their application and usage.
...
the output of all currently available inference methods. 
The detailed genetic genealogy of a sample is defined in terms 
of based on specific genomes
and their intervals of genetic inheritance, which we refer to as 
a ``genome ARG'', or ``gARG''.
This contrasts with the classical Griffiths approach which 
is based on recording evolutionary events, and we therefore refer to as 
the ``event ARG'', or ``eARG'' encoding.
...
ARGs---a necessity under the eARG encoding---should be critically 
examined as part of the process of fully decoupling our ARG data 
structure from the coalescent with recombination model.
...
In this section we provide a  concrete definition of an ARG in terms
of specific genomes and their patterns of inheritance, which
...
In this usage a ``genome'' is equivalent to the definition of a ``haplotype'' used by
\cite{shipilina2023origin}; we avoid term haplotype because 
it already has many interpretations, and in particular carries the implication of 
specific mutational patterns [CITES] which persist over 
many generations. A ``genome'' in our usage corresponds to the 
genetic material present in a particular individual or cell.
...
We consider only patterns of genetic inheritance, with mutational 
processes being entirely separate.
...
where each 
edge\footnote{We use the standard graph theoretic sense of
an ``edge'' here, rather than the somewhat specialised interpretation of \cite{shipilina2023origin}.}
 is annotated with the coordinates
...
as a subset of their pedigree, but
...
from an ancestor) we must ``annotate'' the edges with the genome
...
of local trees along the genome. To generate the local tree at genome
coordinate $x$, we traverse the graph rootwards from the leaves. 
For concreteness, we can describe this tree as an oriented forest $\pi$
\citep{kelleher2013coalescent,kelleher2014coalescent,kelleher2016efficient}.
At each node
$u$, we find an edge $(c, p, I) \in E$ such that $u = c$ and $x \in I$.
We then set $\pi_c \leftarrow p$,  and move to the next node $p$.
...
denoting the fact that \noderef{e} the ancestor of \noderef{a} over the interval $(0, 2]$,
...
% TODO rephrase this, it'll confuse people. Just need to reiterate the
...
ancestral lineages.
The final vital
detail is that we associate a \emph{breakpoint} with each recombination
event, and this information is sufficient to uniquely
define the local genealogical trees at every position along the genome.
...
[TODO: what do we need to cover here?]
...
quirky dependence on SNP data?]

\cite{mathieson2020ancestry} have a slightly different definition of an ARG,
which they say ``is a subset of the pedigree... contain[ing] only those edges along which
inherited segments of DNA have been transmitted, and only those nodes
corresponding to ancestors in which there was a recombination or coalescence
event.'' Thus, while their definition is decoupled from the coalescent
process it is defined by events.
...
ARG that contains
nodes or edges that are not ancestral to the sample
...
(a tree node that has exactly one child), which arise in several different ways.
Unary nodes are not a standard feature of evolutionary trees;
...
provided by unary nodes in the local trees.
...
model. Note that the genome \noderef{k} has three children, and the genomes
\noderef{e} and \noderef{g} simultaneously have multiple parents and multiple
children.
...
that have only one child (e.g.\ \noderef{n}) and nodes that have more
than one child (i.e. are ``common ancestors'') but never represent coalescences
in the local trees (e.g.\ \noderef{r}).
(D) Rewrite edges to bypass any unary nodes in local trees. Note that this loses lineage
information such as the fact that \noderef{k} is connected to \noderef{a} via \noderef{e}.
}
\end{figure}

Fig.~\ref{fig-simplification} show a series of successive steps of
simplification, starting with a complete [TODO be more concrete about
what is stored here; ``complete'' isn't very helpful] gARG simulated under a
backwards-in-time Wright-Fisher
model, shown both as a graph on the left, and the corresponding sequence of
...
that is is possible to \emph{store} varying levels of precision
about recombination in an ARG.
It is also possible, of course, to \emph{infer} varying levels of precision about
recombination.
...
that their statment is true, \emph{if} the local trees contain
