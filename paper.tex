...
Originally, Griffiths and colleagues defined the ARG as an alternative
...
The classical formal definition of an ARG is expressed in terms of
genetic inheritance events: the coalescence and recombination events
...
including the outputs of modern simulation and inference methods.
In particular, we show that in contrast to the eARG encoding which
requires fully precise estimates of each recombination event,
...
in the context of contemporary datasets.
...
between an ARG and its local trees and show (contrary to the
% I want to put a (suitably encoded) here somewhere, but the sentence is
% very long already.
prevailing view) that the sequence of local trees contains
precisely as much information as the corresponding ARG.
The gARG view also opens a rich new set of details about ARGs,
...
which we illustrate by examining the qualitative properties of the
...
Finally, the literature on ARGs and associated aspects is large and
confusing, and we cover some important aspects
...
% We can delete then next two sentances if we want, but nodes are
% integers and this is a vital element of the model.
...
to each of the gARG's edges (Fig.~\ref{fig-ancestry-resolution}B).
...
samples at a given position.

This combinatorial approach provides at least
one novel insight into the relationship between ARGs and local trees.
...
Evolutionary trees usually regard internal nodes as unlabelled
...
local trees.
The ARG inference methods \arbores~\citep{heine2018bridging} and
\espalier~\citep{rasmussen2022espalier} are illustrative in this context.
Both begin by splitting an input sequence alignment into
...
across multiple local trees. \arbores~bridges the gaps
between the inferred local trees via a coalescent-based
sampling scheme.
...
contain information about branching points, and so it is helpful
to define the ``arity''.
...
incorporates uncertainty encountered during inference into the returned ARG.
...
are incorporated in quite an indirect manner through the correlations
