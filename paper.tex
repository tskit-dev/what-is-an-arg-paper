...
Estimating the detailed genealogy of a set of genome sequences
...
Applications using these ancestries as inputs have begun to
...
Originally, the ARG was based on a stochastic
process~\citep{griffiths1991two,ethier1990two,griffiths1996ancestral,griffiths1997ancestral} which modelled the coalescent with recombination \citep{hudson1983properties}. Subsequently, it has come instead to be thought of as a data
structure~\citep{minichiello2006mapping}, i.e.\ a means of
describing the sequence of correlated local trees that are the \emph{outcome} of some such ancestral process. However, the distinction between process and structure is not clear cut and subfields use the term
differently (see Appendix XXX).
%% Sentence deleted: point made already
% ARG inference was for many years synonymous [FIXME: this isn't really true,
% what about parsimony ARGs?]
% with estimating realisations
% of the coalescent with recombination (with, it must be said, limited success),
% and the distinction between process and realisation was not especially important.

%% Repeat of first para
% The recent introduction of two methods, \relate~\citep{speidel2019method}
%and \tsinfer~\citep{kelleher2019inferring}, that estimate structures
%sharing many properties of this classical idea of an ARG, but at previously
%unthinkable scale, has led to a surge of interest in the area.
%
%
%Although both \cite{speidel2019method} and \cite{kelleher2019inferring}
%avoid describing the output of the methods as an ARG,
%and we therefore need a more general definition of an ``ARG'' than the classical
%Griffiths formulation.

The word ``ARG'' is now widely used to refer to genetic genealogies in general~\citep[MORE][]{mathieson2020ancestry}, including
varied outputs from modern simulation and
inference methods~\citep{rasmussen2014genome, palamara2016argon, haller2018tree, kelleher2019inferring, speidel2019method, baumdicker2021efficient, zhang2023biobank}.
However, this broad usage means there is disagreement on the exact object being defined. This causes problems, for example,
not just when evaluating inferred ARGs, but even when defining
...
Perhaps most importantly some definitions of ARGs, especially those linked to a specific stochastic process, are overly narrow which
...
the output of all popular simulation and inference methods, yet simple enough to be encoded via a conventional mathematical graph.
%% Yan: I added the above as both the ARGON and the Shipilova definitions
%% use non-conventional graph structures for their encoding (e.g. hypergraphs, weird edges, etc)
We follow the well established principle that it is advantageous to decouple the presence of mutations from the genealogical structure itself \citep[contra][chap.~3]{gusfield2014recombinatorics}. We propose a definition of genetic genealogy purely in terms of specific genomes
...
which is based on recording evolutionary events, and which we call
an ``event ARG'', or ``eARG''.
We show that the gARG encoding has a range of advantages over the
eARG approach, being both flexible and efficient.
In particular, we argue that the goal of inferring fully precise
ARGs---a necessity under the eARG encoding---should be
% weird phrasing for a criticism?
critically examined
%
as part of the process of fully decoupling our ARG data
structure from any specific generative model.
...
In this usage a ``genome'' is equivalent to the definition of a ``haplotype'' used by~\cite{shipilina2023origin}, which we avoid because
...
many generations. A ``genome'' in our usage corresponds to the
haploid genetic material present in a particular individual or cell.
...
A gARG is then a graph in which nodes represent
haploid genomes, edges represent
genetic inheritance between an ancestor and a descendant, and
annotations specify coordinates of the genomic intervals
over which genetic inheritance occurs.
We can therefore view a gARG for a given set of individuals
as a subset of their pedigree.
An example of this for diploids (so each individual is represented by two nodes) is illustrated in Fig.~\ref{fig-arg-in-pedigree}.
Each genome is inherited from one
...
Genomes \textsf{a} and \textsf{c} are the product of two independent recombination events between
...
The topology of a gARG specifies that genetic inheritance occurred between particular
ancestors and descendants, but the graph connectivity
does not tell us which \emph{parts} of their genomes were inherited.
In order to capture the effects of recombination (and any other process
that results in the partial inheritance of a descendant genome
from an ancestor) we ``annotate'' the edges with the genome
coordinates over which inheritance occurs.
Continuing the example of genome \textsf{a} in Fig.~\ref{fig-arg-in-pedigree},
the recombination at position 2 means that it inherits the (half-closed) genome
interval $[0, 2)$ from \textsf{e} and it inherits $[2, 10)$ from \textsf{f}; these intervals are shown attached to the corresponding graph edges. Alternative gARG encodings split transmitted intervals and associate each with a new node, resulting in a single genome being represented by a group of multiple nodes (or ``blocks'' \citep{palamara2016argon}), with implications for efficiency of storage.

% JK: dunno if this will make some readers switch off, but I feel it's
% important to show that this is a formal model and the concrete definition
% will help some readers?
Formally, our gARG encoding is defined by sets of nodes $N$ and edges $E$, where each
element of $E$ is a tuple $(c, p, I)$ such that $c, p \in N$ are the child and
parent nodes and $I$ is the set of disjoint genomic intervals $[\ell, r)$ over
which genome $c$ inherits from $p$. The most fundamental operation for
an ARG data structure is the process of recovering the corresponding sequence
of local trees along the genome. Concretely, we can describe a local tree
at position $x$ as an oriented forest $\pi = ( \pi_1, \hdots, \pi_{|N|} )$, where the entry $\pi_j$ is the index
of the parent of $j$ and $\pi_j = \emptyset$ if $j$ is not present in the
tree~\citep{kelleher2013coalescent,kelleher2014coalescent,kelleher2016efficient}.
We use a forest to describe the local ancestry at a given position so that we allow for cases where not all leaves have found a most recent common ancestor, resulting in multiple roots.
To construct $\pi$ at position $x$, set $\pi_i = \emptyset$ for $i \in \{1, \hdots, |N|\}$.
Then, traversing over all nodes of the tree,
at each node
$u$, find an edge $(c, p, I) \in E$ such that $u = c$ and $x \in I$, and set
$\pi_c \leftarrow p$.
At the end of this process, $\pi$ fully describes the local tree.

There are some interesting consequences to these definitions. Firstly,
there is
no limitation on mating system, ploidy, or age structure. By making the
...
between genomes (and not causal \emph{events}),
arbitrary patterns of genetic inheritance such as
gene conversions,
multiple simultaneous recombinations (e.g.\ Figure~\ref{fig-arg-in-pedigree}), and so on can be directly expressed.
...
one or more samples find a common ancestor); for example, node \noderef{e}
is present but not coalescent in the left-most tree of Figure~\ref{fig-arg-in-pedigree} (see the ``Precision of recombination information'' section for more
...
In this section we have considered the nodes in a gARG as separate from events in a generative process.
Nevertheless, gARGs are commonly constructed to include only those genomes associated with specific events.
For example, \cite{mathieson2020ancestry} define an ARG as ``a subset of the pedigree... contain[ing] only those edges along which inherited segments of DNA have been transmitted, and only those nodes
corresponding to ancestors in which there was a recombination or coalescence
event.''. In an eARG however, events are even more explicitly embedded in the data structure.
...
In keeping with its original derivation in terms of a backward-time stochastic
process (see Appendix~\ref{sec-arg-history}), the classical definition of an ARG data structure is in terms of events, and we therefore refer to it as an ``event ARG'' (eARG).
In an eARG, nodes represent
common ancestor or recombination events that occurred in the
history of some set of sample genomes, and edges represent ancestral
lineages~\citep{griffiths1996ancestral}.
In a common ancestor event the inbound lineages are merged into a
single ancestral lineage with one parent, whereas in a recombination
event a lineage is split into two independent
ancestral lineages, and a list of two parents is required.
As illustrated in Figure~\ref{fig-arg-data-structure},
the regions of the genome which are transmitted are usually
specified by associating recombination nodes with a breakpoint.
In other words, eARGs traditionally
carry positional information on their nodes, rather than
storing genome positions as edge annotations.

To recover the local tree at position $x$,
requires traversing the graph rootwards from the leaves.
When we encounter a recombination node during traversal
if $x$ is less than the breakpoint we follow
the edge to the first parent, and otherwise follow the edge
to the second parent. This means the order in which parents are
listed is important. Alternatively, additional information can be incorporated to distinguish left and right parents \citep[e.g.][Chap.\ 3]{gusfield2014recombinatorics}.

% Jere & Yan: we thought about going into how simulation is difficult
% with just one recombination node due to needing an order before
% one is available during runtime. But since that can be solved
% fairly trivially with two recombination nodes, we decided it was
% more fiddly than it's worth.
% However, see https://github.com/tskit-dev/what-is-an-arg-paper/discussions/41#discussioncomment-2035609 for additional complexities




% Griffiths and colleagues~\citep{griffiths1997ancestral} originally defined
% the ARG as analog of the Kingman coalescent~\citep{kingman1982coalescent},
% in which the ancestry of a sample of $n$ genomes under idealised
% conditions is modelled. One of the key assumptions of this model is that
% the sample size $n$ is much less than the population size $N$, which means
% that the probability of two multiple concurrent events (i.e., coalescence
% and recombination events) is negligible.
...
\node [anchor=north west] at (8,6) {B};
\node [nodelabel,anchor=north west] at ($(10,5)$) {
\begin{tabular}{c|c|l}
% \multicolumn{2}{c}{Breakpoints}\\
Node & Breakpoint & Parents\\
...
\node [anchor=north west] at (20,6) {C};
\node [nodelabel,anchor=north west] at ($(10,5)$) {
\begin{tabular}{ll}
% Edges & Intervals\\
\hline
...
\end{tikzpicture}
\caption{\label{fig-arg-data-structure}
A graphical depiction of a classical event ARG (left) and a corresponding encoding of the information (right). For uniformity, rather than defining alternative node types, we
identify recombination nodes (e.g.\ \noderef{d}) as those with a non-null breakpoint; these also have two parents. Nodes \noderef{e}, \noderef{f} and \noderef{g} are
common ancestor events.
}
\end{figure}

Storing only breakpoint positions, rather than transmitted regions,
makes it hard to perform genetic analyses (such as extracting local trees)
without traversing the entire graph.
% "For this reason"?
Some eARG encodings, such as \citet{shipilina2023origin}, have a specialised definition of edges, which places them outside the scope of the standard graph-theoretic representation.
%% Is the above true?? It the reason for this to allow efficient tree extraction? Are there other examples where encodings have been
%% built on top of an eARG-like node annotation scheme (ARGON?) and made efficient somehow?

[What is the ARG definition provided by the parsimony ARG literature?]

%[What is Gusfields ARG definition? Presumably it's still an eARG, but with a
%quirky dependence on SNP data?] DONE

Encoding genome positions on edges, rather than breakpoints on the nodes of an eARG, has important implications for efficiency, especially in forward-time simulations (see XXX)


% \section*{Inheritance and ancestral material}\label{Ancestry_resolution}
\section*{Prospective vs retrospective ARGs}
ARGs are usually thought of as a retrospective structure, in which we
...
events that have occurred in the genetic ancestry of a sample.
...
\section*{From eARGs to sample-resolved gARGs}
...
to the samples, \noderef{a}, \noderef{b}, and \noderef{c}. Dashed lines show edges that are
...
revolves around the presence
of ``unary'' regions for nodes
(where a node has exactly one child over a particular genomic region). These arise in several different ways.
...
provided by nodes which appear unary in one or more local trees.
...
(C) Remove nodes that never represent coalescences, i.e. are unary everywhere (e.g.\ \noderef{n} and \noderef{r}).
(D) Rewrite edges to bypass any unary nodes in local trees.
}
\end{figure}

Fig.~\ref{fig-simplification} show a series of successive steps of
simplification, starting with a sample-resolved but unsimplified gARG, simulated under a
...
Consider \noderef{e} in in Fig.~\ref{fig-simplification}A,
...
% \noderef{a} in the leftmost tree (covering positions 0 to 30)
% Consider, for example, the path ancestral to node
% \noderef{a} in the leftmost tree (covering positions 0 to 30)
% in Fig.~\ref{fig-simplification}A.
% We can see from the graph that
% % Before \noderef{a} meets its common ancestor with the other samples at
% % node \noderef{q}, it passes through \noderef{e}, \noderef{j}, \noderef{m}, \noderef{n} and
% % \noderef{p}, which in this region of the genome are all unary nodes.
% % Node \noderef{e}
...
% joining sample \noderef{a} to node \noderef{q} to be equivalent to the path
...
and utilised is an interesting open question [CITE Ralph in prep.].
...
% that vary levels of precision about reccomb
...
information about node sharing across trees, and hence,
in terms of the level of detail inferred about recombination,
lie somewhere
in between a sequence of unrelated trees on one extreme and a
completely precise event ARG on the other.

% \relate\ "postorder" first inferring local topologies and then identifies edges
...
\cite{nowbandegani2023extremely}), where we consider the unique sets of
