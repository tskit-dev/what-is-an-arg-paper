...
\footnotetext[1]{Shared second authorship, listed alphabetically}
...
The eARG definition is based on the stochatic process of the coalescent with recombination,
and we show how the assumptions of that process
place limitations on the patterns of inheritance that can be
represented, which are inadequate for modern large-scale datasets.
The eARG representation also requires complete precision about the ordering
...
and \textsf{f} (with breakpoints in red), and regions of genome inherited are shown.
...
(D) Rewrite edges to bypass nodes where they appear unary in local trees.
}
\end{figure}
...
[TODO discussion of haplotype block based storage
like ARGON. Basically: this is useful for some applications,
but ultimately it's a derived property of the actual ARG
topology that can be recovered efficiently, as demonstrated by 
...
computes selection $p$-values by computing clade size probabilities conditional on the timing of coalescence events in a given local tree.
In their method
...
ground truth data from highly idealised simulations, some 
...
would substantially accelerate progress, and help 
establish ARG methods as mainstream genetics.
...
of the ARG as a discrete graph structure, focussing on the ARG topology without considering
...
stochastic process from the ARG as a data structure. We have followed them in
using the term in the latter sense, to mean a specific realised ancestral history.
...
This may not be obvious.  example, in the gARG encoding depicted in Figure \ref{fig-ancestry-resolution}
...
and thus so is the number of lineages during the time interval. This information cannot be recovered
from the gARG encoding 
