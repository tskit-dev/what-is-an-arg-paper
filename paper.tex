...
and as concrete realisations of ancestry was now broken,
since such parsimonious ARGs are extremely unlikely under the
% JK: I assume this is true; any citations to back it up I wonder?
...
for using the term to mean specific realised ancestral histories.
...
as well as examining more general phylogenetic
...
However, in parallel with this tendancy to decouple the term
...
uses a similar approach to model an ensemble of correlated
potential ancestral trees and has been of considerable
and ongoing theoretical interest~\citep[e.g.][]{
...
koskela2019robust}.
A vital distinction between the ARG (process) and ASG, however, is that there
is that there is a hard distinction between the stochastic process
(the graph) and an observable realisation (a single tree) because
the ASG encodes \emph{potential} ancestral trees.
...
contexts, and in its original form was it was closely coupled to
...
forces spurious precision about the location and timing
