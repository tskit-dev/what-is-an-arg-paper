...
contain many unary nodes, and as we simplify more and more
the local trees contain fewer
unary nodes (Fig.~\ref{fig-simplification}B,C) until
we reach Fig.~\ref{fig-simplification}D in which the local trees
have no unary nodes.
Unary nodes in a local tree arise from
genetic inheritance
passing through an ARG node in which no coalescence
occurs in the corresponding genome interval.
...
local trees to the left of position $44$, and is a unary
node in these three trees. Unary nodes in the local trees
arise from a variety of processes, and would not usually
be considered significant. We are usually interested in tree
