...
% TODO is this (genome) clarification helpful or confusing?
...
genome for a given set of sample genomes.
The relationship between an ARG and its corresponding
local trees is subtle and important. To make the following
discussions precise, we will define a concrete combinatorial encoding
for local trees and an explicit procedure for constructing them.

Oriented trees provide a
convenient formalism to capture these relationships in a well-defined
...
edges $E$, we recover the local tree as follows.
We begin by
setting $\pi_u = 0$ for each $1 \leq u \leq n$. Then, for each sample
...
Note that the trees for an ARG are ``sparse''~\citep{kelleher2016efficient},
...
% I guess we should be a bit more formal here?
...
% Yes, OK, we can rephrase. It is a misunderstanding though
misunderstanding of the relationship between local trees and an ARG.
