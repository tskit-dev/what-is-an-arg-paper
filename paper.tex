...
the miniumum number of recombinations required
to explain an input dataset [CITATIONS], and inferring the corresponding
ARG realisations~\citep{song2005efficient,lyngso2005minimum}.
...
economical description of the coalescent with recombination, but
it has significant drawbacks as the basis for a general purpose
storage format.
...
This is unrealistic because things like gene conversion are common,
and means we can't have back and forth recombination between
two parents.
...
The first method to explicitly produce an ARG as output is SHRUB developed
by \citet{song2005efficient}.
% AI: I think \citep{song2003parsimonious} came before.
...
The limitations of the Griffiths ARG encoding directly stem from its
rooting in the coalescent with recombination stochastic process.
...
% How much more description of the parsimony methods would be good? Maybe a mention of them splitting into
% "back in time" and "across local trees" types?
