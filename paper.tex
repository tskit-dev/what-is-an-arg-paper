...
% TODO finish this sentence with "and more reasons why ARGs are cool"
...
\caption{\label{fig-inferred-args} Inference of an 11 sample ARG over a 2.4kb
region of the \textit{Drosophila melanogaster} ADH locus~\citep{kreitman1983nucleotide}.
...
The graphic below each ARG shows the genomic intervals for each graph edge in the
corresponding ARGs: edges are horizontal lines, stacked in time order
(edges with youngest children at the bottom), with vertical dashed lines indicating
breakpoints between local trees.
}
\end{figure}
...
construct not an ARG but "only" a sequence of local trees \[cite examples from \#38\]. While this statement
...
Fig.~\ref{fig-inferred-args} presents graph representations of genealogies output by two tools that
\emph{do} explicitly infer recombination events, namely \kwarg \citep{ignatieva2021kwarg} and \argweaver
\citep{rasmussen2014genome}, and two which do not (tsinfer and Relate). All inferences are based on the
...
assume a mutation rate of \SI{5.49e-9}{site^{-1}generation^{-1}} and a recombination rate of
\SI{8.4e-9}{site^{-1}generation^{-1}} [CITE stdpopsim] and a constant effective population size of 1000,000.
The version numbers of inference software, together with the specific inference parameters
needed to reproduce the ARGs, are available from our GitHub repository.
...
outputting minimal ones using heuristics, and ARGweaver samples from the posterior distribution over
ARGs given the data and a fixed model (the SMC$\prime$, an approximation of the full coalescent
with recombination). In contrast, \tsinfer and \relate (Fig.~\ref{fig-inferred-args} C \& D) are
...
In Fig.~\ref{fig-inferred-args} A \& B, each recombination node has exactly 2 parents,
corresponding to a single recombination event with a specific breakpoint. Assuming that each
breakpoint occurs a a different location (the "infinite breakpoints" model), then the
number of breakpoints should exactly equal the number of 2-parent (brown) nodes in the plot.
In contrast, Figure Fig.~\ref{fig-inferred-args} C \& D have ARG nodes with greater
than two parents, indicating deliberate imprecision: these are cases where multiple recombination events
have been ``pushed down'' onto a single more recent node. Note, however, that the number
of breakpoints in these two ARGs is smaller than the number of ARG nodes with multiple parents,
because those nodes frequently share breakpoint positions. The sharing of breakpoint locations
between multiple-parent nodes indicates that in \tsinfer and \relate ARGs it may be
difficult to condense each transition between trees into a set of simple SPR moves. 

Shading by coalescent span reveals that many non-recombinant (green) nodes are
``locally unary'' (see section \ref{sec-locally-unary-edges}). In other words,
in some regions of the genome, these nodes are present in the ancestry but do
not correspond to coalescence. As argued previously, this can provide potentially useful
...
Only the \tsinfer results have nodes with more than two children (polytomies);
...
manner to the lack of precision in recombination nodes with more
than two parents. The clearest example is the polytomy involving the nodes
\noderef{Fr-F}, \noderef{Wa-F}, and \noderef{Af-F}, which happen to
have identical sequences. \kwarg, \argweaver, and \relate require
bifurcating trees, so are forced
to pick an arbitrary order of branching
(hence Fig.~\ref{fig-inferred-args} A and Fig.~\ref{fig-inferred-args} B disagree on the
relationship between these samples). TsThe number of nodes in Fig.~\ref{fig-inferred-args} C
that have more than two children also hints at considerable uncertainty in
the order of branching elsewhere in the ARG.
% TODO add explanation of the bottom row.
...
% TODO Jerome: add EPSRC and NIH grants.
