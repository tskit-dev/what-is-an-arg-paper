...
\title{\vspace{-1.5em} \bf A general and efficient representation of ancestral recombination graphs}
...
% FIXME abstract needs some rewriting, we're placing too much emphasis
% on the coalescent there.
\begin{abstract}
...
and require complete precision about all recombination events.
...
impede progress in applying genetic genealogical
...
For a recent and more biologically oriented introduction to
ARGs, see \citet{lewanski2023era}.
...
positions. Individual $D_1$ is therefore composed
...
by the evolutionary \emph{events} in the history of a sample of genomes.
...
% confuse people
Thus, we have the means to encode the \emph{effects} of
...
to have  any number of children; from one (see the next section for more
discussion on such ``unary nodes''), to arbitrarily
large polytomies.
...
using only the tree topologies it would not be straightforward to
infer that node \noderef{i} in the first and last trees of
Fig.~\ref{fig-arg-in-pedigree}C is the same node.
...
(see the next section for discussion on this point).
However, if the local trees
contain labelled internal nodes,
...
sequentially along the genome, and subsequently stitching them together
into an ARG.
...
\noderef{g} and \noderef{h} are graph-unary, and are consequently
also unary in the local trees.
On the other hand, node \noderef{f} has two children
in the graph (Fig~\ref{fig-arg-in-pedigree}B), but is only binary
...
Outside of the context of simulation, graph-unary nodes are
...
little information about these intermediates, or utility
in modelling them.
...
which we illustrate by an example simulated ARG, which
undergoes a series of successive steps of simplification
...
backwards-time Wright-Fisher simulation for a sample of 2 diploid
...
the \emph{effects} of a recombination via outbound edges to
the parent individual's two genomes.
...
the graph depicted on the left and corresponding sequence
...
nodes) correspond to the size of the inheritance intervals
...
material through genomes under a range conditions.
...
not possible to infer precisely from sequencing data. For ARGs under
...
even more challenging. The fact that the eARG encoding \emph{requires} completely
...
In most cases,
ARG inference is evaluated by simulating data from a known ground truth ARG, and comparing this to the inferred version via pairwise comparison of local trees along the genome, for example
using tree distance
...
distributions of pairwise MRCA times; \citet{deng2021distribution} derived an approximation to the distribution of the
waiting distances between local trees; \citet{ignatieva2023distribution}
