...
An ARG is classically defined as a graph of
% We can play a bit loose here, please don't change to "common ancestor"
coalescence and recombination events in the history of a sample of genomes,
encoding an inheritance structure where
crossover breakpoints are associated with recombination events.
We show that this ``event ARG'' (eARG) encoding
is limited in the patterns of genetic inheritance that can be represented,
being insufficient to describe the outputs of recent ARG simulation
...
We show that the gARG encoding can fully capture the richness of large-scale datasets
and is compatible with modern simulation and inference methods.
We also show that, unlike the eARG encoding which requires
% TODO rephrase "to be detailed" is ambiguous
all recombination events to be encoded,
...
and ultimately an entire species or higher grouping. 
This ARG is the true history,
...
Just as this true ARG unquestionably exists,
it can also never be entirely known.
% TODO: I'm not happy with the phrasing here, anyone want to jump in?
Even if the mutation rate were high enough to identify every
recent branching point uniquely (while remaining biologically survivable),
such a high rate would saturate the genome with mutations, inevitably obscuring
past history.
% Must be a citation for this?
Similarly, it is unlikely that the details
of ancient recombinations can ever be known with much precision.

% Popgen models and the species ARG
Species-level ARGs may seem fanciful, but the scale of
...
and exome~\citep{backman2021exome} and
whole genome sequence~\citep{halldorsson2022sequences} data
for large subsets of this cohort.
UKB is just one of many such projects~\citep[e.g.][]{turnbull2018hundred,
karczewski2020mutational,tanjo2021practical}, and
agricultural datasets are even
richer in terms of generations~\citep[e.g.][]{hayes20191000,Ros-Freixedes2020}.
While we are not sequencing entire species,
we are certainly sampling at a scale where the assumptions of
population genetics models are substantially broken.
By necessity and design, such mathematical models are caricatures
of biological reality,
allowing us to draw general conclusions about population processes
without the burden of unnecessary detail.
 % TODO we're singling out the coalescent here, but it's just an 
 % exmaple. The DTWF is no better an approximation of the last 5
 % generations of UKB, e.g.
The coalescent with recombination (henceforth, ``coalescent'')
is a powerful model that defines a distribution over ARGs
for a given set of parameters.
A key assumption of the coalescent is that
sample size is much less than the effective population size ($N_e$)
and although the model has proved surprisingly robust to violations
of this assumption~\citep{wakeley2012gene,bhaskar2014distortion,
nelson2020accounting},
sample size can now be an order of magnitude or more \emph{larger} than the
...
ARGs sampled from the coalescent cannot capture the full complexities
of family structure or the vagaries of history inherent in these
datasets.

% What is the problem?
The true ARG underlying a modern population-scale dataset
therefore cannot be known in its entirety, and the value
% TODO rephrase heavily!
of attempting to describe the intricate details of the recent past
through mathematical models is questionable.
A pragmatic goal that we may set, then, is to infer \emph{aspects}
of the true ARG and to capture uncertainty where it inevitably exists.
This is the approach taken by recent inference
% TODO should we mention SARGE? I don't think it's in the same class, but
% maybe I'm wrong?
methods~\citep{kelleher2019inferring,speidel2019method,zhang2023biobank},
which estimate an approximate version of the true
ARG, informed, but not strictly constrained, by population genetic
models and parsimony.
While these methods have made substantial progress there
are, however, some significant problems in this emerging field.
Our terminology and mathematical formalisms for ARGs,
ultimately derived from the coalescent,
are ill-suited to describing these approximate structures.
% TODO add some citations of examples?
Discussions are forced to contrast a ``full'' ARG, containing
a precise description of every recombination or coalescence event,
with a sequence of disconnected local trees.
This view is depauperate, obscuring many shades of important
and interesting detail between these two extremes.
...
zhang2023biobank},
there is no corresponding mathematical description of an ARG data structure.
Classical formalisms, based on the Griffiths approach, are insufficient to
describe these rich details.
Given that ARGs have already been estimated for the 500,000
humans in UKB~\citep{kelleher2019inferring,zhang2023biobank} and
more than a million SARS-CoV-2 genomes~\citep{zhan2023towards},
it is vital that our formal methods and terminology catch up
with this reality.

We address these problems here by providing a simple formal description of an
...
flexibility in how genetic inheritance can be represented, and encompassing the
...
% Do we need to say this? Probably not?
We are not questioning the form of the true ARG, or its existence.
...
individual. How we assign dates will therefore vary according to application.
...
for a recombination event). The fundamental problem with this approach
is that it assumes that all events are \emph{knowable}, and does not
