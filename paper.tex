...
% It would be slightly easier for us if we defined nodes as in N_0, since
% we use 0 as a node ID, but leaving for now.
...
% Because of our definitions we can't have anything to the left of 0,
% so you can treat breakpoint at 0 as no recombination.
$\sigma(u) = 0$ otherwise. For each recombination node $u$ we must
...
position $x$ by the following procedure. Begin by initialising
a stack
...
We can define a GARG as a set of directed edges $E$, where each
element of the set is a tuple $(c, p, I)$ such that $c, p \in \mathbb{N}$,
$c$ is the child node, $p$ is the parent node and $I$ is the set of
disjoint genomic intervals $(\ell, r]$ over which genome $c$ inherits from $p$.
...
% This is weak. Should we give a better description of the algorithm?
% Not clear yet, and don't want to spend too much time on it if it's
% unhelpful.
...
% weak and not correct - how do we say this properly?
