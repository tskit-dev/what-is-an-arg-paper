...
recombinant ancestry of a sample of genomes.
\end{abstract}
...
ARG is known in complete detail: they
are not suited to describing partial or approximate knowledge.
As we are actively inferring ARGs at the population scale,
and such ARGs can never be fully known,
there is currently a substantial gap between classical representations
...
% This is a digression from the main point of the paragraph, so
...
far more than the seven required for this dataset under minimal parsimony
...
with projects under way to obtain
...
choose \emph{one} best guess, if sample sizes are large
it is not clear that generating many guesses will achieve much.
...
describe and use uncertainty about ARG inference, and to
...
How this uncertainty can be used in downstream applications
is an open question.
