...
metadata such as a sample identifiers or quality-control metrics
...
shown, along with their constituent genomes and paths
of genetic inheritance.
We label nodes using using lowercase alphabetical letters
rather than integer identifiers to avoid confusion with genomic
positions and associated intervals.
...
Locally unary nodes are a vital link between ARGs and local trees, and we
...
contain locally unary nodes, and state that paths are
...
``pass-through'' nodes with one parent and one child, and in practice these would normally be omitted.
