...
as a process along the genome rather than backwards in time.
...
% FIXME there should be earlier refs for SPRs, right?
operation~\citep{song2003on,song2006properties}.
The number of SPRs required to transform one leaf-labelled tree
into another is known as the SPR distance [cite]
and is computable in O(??) time.
...
it is not obvious how to produce an ARG given (say) a
collection of newick trees. If the internal nodes are
labelled, then we can simply input the edges for each
tree in turn, which can easily be ``squashed'' down
to a minimal representation. However, internal nodes
are rarely labelled in newick trees, and therefore
the process of contructing a graph is much harder.
In princple, we can contruct an EARG by reasoning
about the SPRs that must has occured between two
adjacent trees and postulating the recombination
nodes that must have occured. However, this process
is computationally expensive [How expensive? is this
what Espalier does?] Additionally, unless some extra
information is provided the time of the recombination
will not be known. Fundamentally, we must include the
unary nodes corresponding to recombination parents
in the trees if we wish to recover an ARG from them.
