...
\section{Event ARGs}
\label{sec-eARG}
In this section we define a common (and classical) view of an ARG data structure,
...
for notable exceptions), and used as an output format by multiple ARG inferences tools
...
% down.
In this encoding we have two types of internal node in the graph,
...
we will refer to this encoding as the ``event ARG'' or eARG
encoding. Fig.~\ref{fig-event-arg} shows an example of an
...
3 PARAS. - enough outline to let readewr know what's in the appendices.

There are lots of new inference techniques: they all produce slightly different
structures, all of which can be though of a types of ARGs...
...
These give different levels of precision about recombination (see new Appendix X)

You can see this as different levels of detail on the same underlying truth. Here's a figure:
...
THE FIGURE OF INFERRED ARGS WAS HERE
