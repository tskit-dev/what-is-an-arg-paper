...
From a high-level, ARGs describe the sequence of correlated genealogical
trees along the genome, induced by the different paths of
genetic inheritance caused by recombination.
...
many generations.
% A ``genome'' in our usage corresponds to the
...
If $u$ is a root
% I'm not sure if this is a helpful clarification, and whether it'll just
% confuse people. It depends on some tricky details.
(or not reachable from the samples)
in the local tree at position $x$, $\pi_u = -1$.
...
large polytomies.
To recover the local tree $\pi$ at position $x$  then, we begin by
...
(e.g.\ Figure~\ref{fig-arg-in-pedigree}),
...
% are not explicitly concerned with ``coalescence'' (nodes at which
% one or more samples find a common ancestor); for example, node \noderef{e}
% is present but not coalescent in the left-most tree of
% Figure~\ref{fig-arg-in-pedigree}
% (see section XXX for more discussion).
...
Genome ARGs model the inheritance between specific genomes,
annotating the edges of the graph with the intervals
of genetic inheritance. It is primarily concerned with
describing relationships and the details of inheritance
between pairs of genomes.
In contrast, the classical ARG data structure is defined
by evolutionary \emph{events},
and we therefore refer to it as an ``event ARG'' (eARG).
The difference between gARGs and eARGs is subtle and the distinctions
at times rather porous. In essense, our main goal of introducing the
term ``Event ARG'' is to contrast the flexibility of the genome ARG
view with current practice. It is therefore useful to consider
the definition of an ARG from recent work.

% Todo this is rough. Ideally we want a bit of commentary running through
% the quotations.
\cite{mathieson2020ancestry} define an ARG as ``a subset of the pedigree...
contain[ing] only those edges along which inherited segments of DNA have been
transmitted, and only those nodes corresponding to ancestors in which there was
a recombination or coalescence event.''
\citet{shipilina2023origin} emphasise the reality  ``The ARG describes the complete
ancestry of a sample of genomes through a series of real coalescence and
recombination events''. In contrast,
\citet{brandt2021evaluation} tie the definition of an ARG explicitly
...
In contrast ~\citet{gusfield2014recombinatorics} defines an ARG as [FIXME]
...
The common features of these definitions is that they are expressed
in terms of evolutionary events, emphasise the exhaustiveness
of the record (full, complete), and they do not specify the mechanism
by which partial genetic inheritance occurs. Just as the graph topology
in a gARG defines the paths along which genetic inheritance occurs, some
additional information is required to define the actual regions of genome
inherited. 

In the classical view of an eARG we usually have two different types
of node,
representing common ancestor and recombination events that occurred in the
history of some set of sample genomes
and edges represent ancestral lineages~\citep{griffiths1996ancestral}.
In a common ancestor event the inbound lineages are merged into a
single ancestral lineage with one parent, whereas in a recombination
event a lineage is split into two independent
ancestral lineages with an associated breakpoint.
Figure~\ref{fig-arg-data-structure} shows an example of a classical
...
We use the standard convention here
that the order of parents
is significant, but additional information can be incorporated to
distinguish left and right parents
\citep[e.g.][Chap.\ 3]{gusfield2014recombinatorics}.

Beyone the interpretation of nodes as representing events or
genomes (which may reasonably be seen as somewhat philosophical),
the essential difference between a gARG and a classical eARG
[JK WIP from here:]
is that eARGs define the
...
\end{tikzpicture}
\caption{\label{fig-arg-data-structure}
A graphical depiction of a classical event ARG (left) and a corresponding
encoding of the information (right).
Recombination nodes (e.g.\ \noderef{d}) have a non-null breakpoint
and two parents.
...
\citep{mathieson2020ancestry} show examples in which the ARGs 
...
trees~\citep[e.g.][]{minichiello2006mapping} [more]
