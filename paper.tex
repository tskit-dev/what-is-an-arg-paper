...
output of four methods run on the 
\cite{kreitman1983nucleotide} dataset, often used as an
ARG inference benchmark. One source of differences between 
the inferences is the underlying model. For instance, 
the ARG in Fig.~\ref{fig-inferred-args}A is inferred by 
\kwarg\ under parsimony, and therefore has many fewer 
ancestral nodes then Fig.~\ref{fig-inferred-args}B, which 
is sampled from the coalescent with recombination with an 
appropriate recombination rate parameter. These differences
are well understood.
...
The differences between ARGs that we wish to draw attention to here 
are more subtle

As is well-known (cite, perhaps Gusfield?), many different ancestral graph
topologies are compatible with the same resulting sequence data. Hence it is
unsurprising that the four graphs differ, for example in the numbers of nodes
and their relationship to each other. However, more detailed inspection of
Fig.~\ref{fig-inferred-args} reveals a diversity of representation that is only
possible via the gARG encoding. For instance, although the most parsimonious
ARG in Fig.~\ref{fig-inferred-args}A has far fewer ancestral events than an ARG
instance sampled from from the coalescent with recombination in
Fig.~\ref{fig-inferred-args}B, both consist only of nodes with either two
parents or two children (but not both). These nodes correspond to recombination
and common ancestor events in the equivalent eARG. In contrast, more scalable
heuristic inference methods (Fig.~\ref{fig-inferred-args}C and D) create ARGs
in which a node can simultaneously have more that than two children and more
than two parents. Note also that the first three methods infer nodes that have
a ``coalescence span'' of less than 100\%, indicating that in the local trees,
ancestral lineages exist that pass through these nodes without the nodes
marking a coalescent point. Such ``locally-unary'' nodes are an notable feature
of the local trees defined by an ARG (see Appendix
\ref{sec-locally-unary-edges}).

These differences, in particular the number of parents and children, and the
coalescence span, reflect the degree of precision represented in the inferred
ARGs. Indeed, it is important to note that the gARG encoding allows a single
underlying genetic genealogy to be represented at differing levels of detail.
The concept is illustrated in Fig.~\ref{fig-simplification}, which shows
...
single parent (Fig.~\ref{fig-simplification}C, also see
Appendix~\ref{sec-cell-lineages-and-args}). Finally, if we chose to represent
only local coalescence points (Fig.~\ref{fig-simplification}D), we end up with
a ``fully simplified'' ARG of the sort inferred in
Fig.~\ref{fig-inferred-args}D. Note that in all cases, the equivalent trees
only differ in the locally-unary nodes they contain; topological relationships
