...
(Appendix~\ref{sec-cell-lineages-and-args}).
By definition, these chains of single-parent inheritance give rise,
over time, to a tree structure.
...
branching points in the local tree (such as \noderef{e} in Fig.~\ref{fig-arg-in-pedigree}),
...
interval: in other words it is a \emph{locally unary} node.
In another example, \noderef{e} is binary in the graph, being a common
ancestor of \noderef{a} and \noderef{c}, but is
locally unary in all trees in which it is present.
This is because no ancestral material coalesces there:
