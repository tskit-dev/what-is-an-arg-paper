...
% We show that a synthesis of these views can be achieved by thinking
% in terms of genomes and transmission
% intervals.
...
We argue that classical ARG formulations, which assume
that all events can be known and precisely estimated,
are fundamentally unsuited to the realities
of modern, population-scale datasets.
We describe an alternative formulation that
...
ancestry
(see \citet{lewanski2024era,brandt2024promise} for biologically oriented
introductions).
...
methods are all based, in different ways, on inferring
approximate \emph{structures}
instead of a complete and fully detailed history
(Appendix~\ref{sec-survey-arg-infer}).
...
We highlight this problem and show how to address it by a simple formal definition of an
...
describe uncertainty about the temporal ordering of multiple recombinations.
...
\section{Event ARGs}
\label{sec-eARG}
A common, classical view of an ARG data structure,
...
This eARG encoding is the basis of the output formats created by multiple ARG inference tools
...
% down.
In this encoding there are two types of internal node in the graph. These
represent the common ancestor and recombination events
in the history of a sample.
...
known, so it is disadvantageous to use a data structure that enforces complete
precision.
...
There are a number of tools for inferring genetic genealogies,
and the graph structures that they produce differ in important aspects.
These varying representations have all been termed ``ARGs'' by different authors,
and each describes more than a simple sequence of uncorrelated trees.
By way of illustration, Fig.~\ref{fig-inferred-args} depicts the output of four
different ``ARG inference'' tools run on a common dataset,
with the resulting ARGs converted into a standard gARG format (cite tskit).
...
(A) \kwarg; (B) \argweaver; (C) \tsinfer; and (D) \relate, converted to the standard \emph{tskit} gARG encoding. See
...
As is well-known (cite, perhaps Gusfield?), many different ancestral graph topologies are compatible with the same resulting
sequence data. Hence it is unsurprising that the four graphs differ,
for example in the numbers of nodes and their relationship to each other.
However, more detailed inspection of Fig.~\ref{fig-inferred-args} reveals a
diversity of representation that is only possible via the gARG encoding.
For instance, although the most parsimonious ARG in Fig.~\ref{fig-inferred-args}A
has far fewer ancestral events than an ARG instance sampled
from from the coalescent with recombination in Fig.~\ref{fig-inferred-args}B,
both consist only of nodes with either two parents or two children (but not both).
These nodes correspond to recombination and common ancestor events in the equivalent eARG.
In contrast, more scalable heuristic inference methods (Fig.~\ref{fig-inferred-args}C and D)
create ARGs in which a node can simultaneously have more that than two children and more than two parents.
Note also that the first three methods infer nodes that have a
``coalescence span'' of less than 100\%, indicating that in the local trees, ancestral lineages exist 
that pass through these nodes without the nodes marking a coalescent point. Such ``locally-unary''
nodes are an notable feature of the local trees defined by an ARG
(see Appendix \ref{sec-locally-unary-edges}).

These differences, in particular the number of parents and children, and the coalescence span, reflect the
degree of precision represented in the inferred ARGs. Indeed, it is important to note that the gARG encoding
allows a single underlying genetic genealogy to be represented at
differing levels of detail. The concept is illustrated in Fig.~\ref{fig-simplification},
which shows different levels of ``simplification'' (Appendix~\ref{sec-ARG-simplification}) of the
same underlying simulated ARG. 
The full ARG, with all coalescent and recombination events represented by
separate genomes, is shown in Fig.~\ref{fig-simplification}A. Simpler representations can be formed by removing ``unknowable'' nodes (Fig.~\ref{fig-simplification}B) and 
collapsing multiple recombinations into a single child or multiple coalescences into
a single parent (Fig.~\ref{fig-simplification}C, also see Appendix~\ref{sec-cell-lineages-and-args}).
Finally, if we chose to represent only local coalescence points (Fig.~\ref{fig-simplification}D),
we end up with a ``fully simplified'' ARG
of the sort inferred in Fig.~\ref{fig-inferred-args}D. Note that in all cases, the equivalent
trees only differ in the locally-unary nodes they contain; topological relationships
between the samples remain identical.
...
%%% Pasted from previous para

The ability to represent an ARG to differing degrees of precision is a powerful
feature of gARGs. In particular, when inferring ARGs from genome
...
representing uncertainty over the ordering of coalescence events, 
Appendix~\ref{sec-cell-lineages-and-args}) or
more than two parents (representing uncertainty over the ordering
of multiple recombination events, Appendix~\ref{sec-ARG-simplification}). 
...
the ARG used in Fig.~\ref{fig-ancestry-resolution} as well as
the simulation code and functions
used to generate Fig.~\ref{fig-inferred-args} and Fig.~\ref{fig-simplification}.
This includes the software versions, parameter settings, and
...
Fig.~\ref{fig-simplification}A and Fig.~\ref{hudson_vs_bigARG},
...
can hope to infer from sampled genomes, which is the topic of
Appendix~\ref{sec-precision}.
