...
Two recombination breakpoints, at positions 3 and 6 mean that genomes \textsf{A}
and \textsf{C} are independent recombinants of the paternal genomes \textsf{E}
and \textsf{F}: the region of genome transmitted through each edge is shown in
these cases. Where genomes merge into a common ancestor, the merged region
is shown darker, until all genomic regions are fully coalesced in genome \textsf{N}
...
breakpoint at position 3, is associated with two parent genomes (\textsf{E} and 
\textsf{F}) and a child genome (the recombinant, \textsf{A}).
