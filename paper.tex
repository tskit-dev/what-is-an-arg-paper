...
we have no information about it, given these sampled genomes.
...
Describing this instead as a ``prospective ARG'', and emphasising
the use of the same data structures and direct connection with
...
(This example of an eARG
from \citet{wiuf1999recombination} is something of a straw-man,
in that it is highly unlikely that an inference or simulation
program would produce an ARG that contains nodes or edges
that are not ancestral to the sample.)
More generally, the gARG encoding allows for ``local'' inspection
(with respect to a set of samples)
