...
% schematic~\citep[][Fig. 5]{hudson1983properties}, % but for clarity and following recent usage,
...
%% NB: I (yan) have commented out the following section as the "latent parameter" quote is
%% used again in the "ARG reconstruction" section, and may be more detail than is required here
%% This also loses the description of ARG space being large etc.
...
hence edges either span the entire genome, or all the region to the left (or right) of a breakpoint
...
Fig~\ref{fig-ancestry-resolution} illustrates the two different edge annotation schemes.
To obtain the sample-resolved annotation in Fig~\ref{fig-ancestry-resolution}B we must
traverse backwards in time from sample nodes \textsf{a}, \textsf{b}, and \textsf{c}, using the
edge intervals to record regions of sample genome that have passed through each route.
When edges join into a parent node, the parent is allocated the union of the intervals.
When a child node is connected to two parents, one parent is allocated the edge
intervals of the child, truncated to include only genomic material to the left of the breakpoint.
Similarly the other parent is allocated the child's intervals truncated such that they contain
only regions to the right of the breakpoint (a natural extension exists for multiple
...
transmitted immediately between parent and child: i.e. local to those two nodes in the tree.
...
% NB: below is a discussion of the Hudson algorithm which we might want to
% retain in an appendix
...
substantially more efficient than immediate edge annotation. Second, by the time we reach
...
multiple root nodes in different regions of the genome. Third, there exist nodes such as
