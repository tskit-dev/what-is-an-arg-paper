...
follow~\citep{harris2019database,harris2023using}. \citet{lewanski2023era} give a more biologically oriented introduction to ARGs.
...
general~\citep[][AND OTHERS]{mathieson2020ancestry}, including
...
% et al's edges, and also get "Graph theory" in here
...
% TODO mention Yamato here? I'm sure there were some interesting points
% about local trees in that paper
...
trees~\citep[e.g.][AND OTHERS]{griffiths1996ancestral,minichiello2006mapping}
...
% JK: Relate is inferring trees independently within segments,
% it's just using a dynamic algorithm to decide what those segments
% are, right? That's how it differs from things like Espalier which
% assume a set of input recomb breakpoints.
...
% JK: I'd usually use "JK was supported by XX", but this form seems fine too?
% I don't have an opinion about it, if others think something else is more
% appropriate.
...
wang2009population, fallon2013acg, mahmoudi2022bayesian}.
...
\citep{kuhner2017consensus} provide an approach to generate a
...
coalecent process [AND OTHERS]\citep{wakely2008coalescent}.

At the base of this concept is a subtle and important point about which genomes the nodes in a gARG represent. 
In principle, we can interpret nodes as representing genomes
...
and correspond to the processes of meiosis and mitosis respectively. A comprehensive gARG would therefore
trace the process of inheritance not between diploid individuals, but between each cell during the
life cycle of an organism. This is illustrated in Fig.~\ref{fig-cell-lines}, where we show a simplified
...
four sampled chromosomes. The potential ARG nodes are shown as green chromosomal regions, and the ``events''
underlying the ARG are shown as red and blue squares.

The unusual feature of this particular example is that three ARG lineages coalesce in one individual, $D_10$
(who has three children). We know that the underlying coalescence process must consist of two successive bifurcations
as the only known method of reproducing DNA is by (semi-conservative) duplication. However, if we take each node in the
ARG to represents the generalised genome of an individual, rather than the specific genome of a cell within an individual,
we can only represent this as a instantanous trifucation or polytomy.

There is biological significance to this cellular-level inheritance process. Were a mutation to occur
during the first cell division of $D_10$, the two gametes produced by the cells
in the left half of $D_10$ could share a mutation not present in the right hand gamete. In other words,
...
relationships will be impossible to infer from the data, with subsequent implications for
using the eARG representation to store the results of inference.
...
(highlighted as deep red and blue squares within individuals $D_5$, $D_10$, and $D_13$).
